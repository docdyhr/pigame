...
char *get_version(void) {
    ...
    if (script_dir) {
        snprintf(
            version_file, sizeof(version_file), "%s/src/VERSION", script_dir);
    } else {
...
static const char *PI_DIGITS =
    ...
    "6"
    "20899862803482534211706798214808651328230664709384460955058223172535940812"
    "8"
    ...
    "0"
    "46652138414695194151160943305727036575959195309218611738193261179310511854"
    "8"
...
char *calc_pi(int length) {
    ...
    LOG_D("calc_pi: result = '%.*s...'", length > 10 ? 12 : length + 2, result);
    return result;
}
...
int main(int argc, char *argv[]) {
    ...
    if (optind < argc) {
        ...
        /* Easter egg */
        if (strcmp(your_pi, "Archimedes") == 0 || strcmp(your_pi, "pi") == 0 ||
            strcmp(your_pi, "PI") == 0) {
            LOG_D("easter egg triggered with '%s'", your_pi);
            printf("π is also called Archimedes constant and is commonly "
                   "defined as\n");
            printf(
                "the ratio of a circles circumference C to its diameter d:\n");
            printf("π = C / d\n");
            return 0;
        }
